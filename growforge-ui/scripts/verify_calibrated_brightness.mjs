...
async function run() {
  console.log("[Test] Launching Chrome for Calibrated Brightness & Idle Stability Verification...");
  const chromeProcess = spawn(
    ...
    [
      ...
      "--user-data-dir=" + path.join(ARTIFACTS_DIR, "scratch", "chrome_temp_calib"),
      "http://localhost:3000",
    ],
    { stdio: "ignore" }
  );
  ...
  try {
    ...
    // Initial Default View Screenshot
    await cdp.captureScreenshot("nucleus_calibrated_default.png");

    // Close-up Screenshot (optical FOV zoom)
    console.log("[Test] Setting optical FOV zoom for close-up...");
    await cdp.eval(`
      (() => {
    ...
    `);
    await sleep(1500);
    await cdp.captureScreenshot("nucleus_calibrated_closeup.png");

    // Record 4s WebM video of close-up nucleus
    console.log("[Test] Recording canvas (4s) for nucleus_calibrated_recording.webm...");
    await cdp.eval(`
      (() => {
    ...
    if (videoDataUrl) {
      ...
      const videoPath = path.join(ARTIFACTS_DIR, "nucleus_calibrated_recording.webm");
      fs.writeFileSync(videoPath, Buffer.from(base64Data, "base64"));
      console.log(`[Test] Video saved: nucleus_calibrated_recording.webm (${Buffer.byteLength(base64Data, "base64")} bytes)`);
    }
    ...
    for (let s = 0; s <= 12; s++) {
      ...
      if (sample) {
        samples.push(sample);
        console.log(`[Sample t=${s * 5}s] InnerScale=${sample.innerScale?.toFixed(3)}, InnerOpacity=${sample.innerOpacity?.toFixed(4)}, OuterOpacity=${sample.outerOpacity?.toFixed(4)}, uPulse=${sample.nucleusUniforms?.pulse?.toFixed(4)}`);
      }
      if (s < 12) {
        await sleep(5000);
      }
    ...
    // 60s Screenshot
    await cdp.captureScreenshot("nucleus_calibrated_60s.png");
    ...
    fs.writeFileSync(
      path.join(ARTIFACTS_DIR, "nucleus_calibrated_metrics.json"),
      JSON.stringify(metrics, null, 2)
    );

    console.log("\n========================================");
    console.log("[Test] CALIBRATED BRIGHTNESS VERIFICATION RESULTS:");
