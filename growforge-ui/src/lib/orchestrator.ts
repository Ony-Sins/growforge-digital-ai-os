...
import { withDurableRetry, dispatchDurableJob, recoverInterruptedJobs } from "@/lib/durableJobEngine";
 ...
 *
 * Durability (Phase 4, Item 5):
 * - Decomposed into durable stage executions with exponential-backoff retries.
 * - Backed by Trigger.dev v3 background task definitions and local crash-recovery engine.
 * - Process restarts / crashes automatically resume in-flight jobs from checkpointed state.
 */
...
export function createAndStartJob(brief: string, createdBy?: string): Job {
  ...
  // Dispatch via durable background engine (handles retries, isolation & checkpointing)
  dispatchDurableJob(job.id, resumePipeline);

  return job;
}

export async function runPlanStage(job: Job): Promise<Plan> {
  return withDurableRetry(`plan (${job.id})`, async () => {
...
export async function runReconcileStage(jobId: string, drafts: Draft[], dossier: Dossier): Promise<string> {
...
export async function runQaStage(jobId: string, drafts: Draft[], review: string, dossier: Dossier): Promise<string> {
  return withDurableRetry(`qa (${jobId})`, async () => {
...
export async function runFinalStage(jobId: string, drafts: Draft[], review: string, qa: string, dossier: Dossier): Promise<string> {
  return withDurableRetry(`final (${jobId})`, async () => {
...
export async function resumePipeline(jobId: string): Promise<void> {
  ...
  const finalOutput =
    stepStatus("final") === "pending" ? await runFinalStage(jobId, drafts, review, qa, dossier) : getJob(jobId)!.finalOutput!;
...
Return JSON exactly in this shape:
{
  ...
  // Re-run through durable dispatch engine
...
if (typeof process !== "undefined") {
  try {
    recoverInterruptedJobs(resumePipeline);
  } catch (err) {
    console.warn("[orchestrator] Startup crash recovery check skipped:", err);
  }
}
