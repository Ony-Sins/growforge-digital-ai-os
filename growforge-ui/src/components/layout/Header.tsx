...
export function Header({ user }: { user: HeaderUser | null }) {
  const { openAdminDrawer, setActiveView, avatarUrl, uiMode } = useAppState();
    ...
    <header className="relative z-40 flex h-16 shrink-0 items-center gap-4 border-b border-border-metal bg-white/70 px-4 backdrop-blur-xl md:px-6">
      ...
      <div className="flex min-w-0 items-center gap-2 text-sm">
        <button
          ...
          className="flex shrink-0 items-center gap-2 rounded-lg px-1.5 py-1 transition-colors hover:bg-sunken"
        >
      ...
      <div className="ml-auto flex items-center gap-3">
        {/* Admin Drawer Toggle Button — Advanced mode only. A business
         *  user in Simple mode never needs this in view. */}
        {uiMode === "advanced" && (
          <button
            type="button"
        ...
        <div className="relative">
          ...
          {notifOpen && (
            <>
              ...
              <div className="absolute right-0 top-full z-50 mt-2 w-72 rounded-xl border border-border-metal bg-white p-2 shadow-2xl">
                {pendingCount === 0 ? (
                  ...
                  <ul className="space-y-1">
                    {pending.approvals > 0 && (
                      <li className="rounded-lg px-2 py-2 text-xs text-secondary">
                        <span className="font-semibold text-navy">{pending.approvals}</span> real-action approval{pending.approvals === 1 ? "" : "s"} waiting — see the gold banner at the top of the screen.
                      </li>
                    )}
                    {pending.consultations > 0 && (
                      <li className="rounded-lg px-2 py-2 text-xs text-secondary">
                        <span className="font-semibold text-navy">{pending.consultations}</span> operator question{pending.consultations === 1 ? "" : "s"} waiting — check the HITL drawer.
                      </li>
                    )}
                    {pending.unapprovedPlans > 0 && (
                      <li>
                        <button
                          ...
                          className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left text-xs text-secondary hover:bg-sunken hover:text-navy"
                        >
                          <span>
                            <span className="font-semibold text-navy">{pending.unapprovedPlans}</span> plan{pending.unapprovedPlans === 1 ? "" : "s"} finished, not yet approved
                          </span>
        ...
        >
          ...
          <div className="hidden flex-col leading-tight lg:flex">
            <span className="truncate text-xs font-semibold text-navy">Profile</span>
